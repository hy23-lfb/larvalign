function h_traverse_and_find_tif(rootpath)
...
if (no_of_channels == 3)
    M           = uint16(zeros(height, width, no_of_channels, frames));
    for slice = 1:frames
        tiff_link.setDirectory(slice);
        M(:,:,:,slice) = uint16(tiff_link.read());
    end
    N               = M(:,:,3,:); % NP channel
    Max_N           = max(N, [], 4);
elseif (no_of_channels == 1)
    fprintf("Channel 1\n");
    M           = uint16(zeros(height, width, frames));
    for slice = 1:frames
        tiff_link.setDirectory(slice);
        M(:,:,slice) = uint16(tiff_link.read());
    end
